 ...
 * @abstract
 */
abstract class Avangpress_Integration {
	...
	protected function get_default_options() {
		...
		if( ! $defaults ) {
			$defaults = require Avangpress_PLUGIN_DIR . 'config/default-integration-options.php';
		}
	...
	protected function parse_options( array $options ) {
         ...
         * @ignore
		 */
		return (array) apply_filters( 'avangpress_' . $slug . '_integration_options', $options );
	}
	...
	public function print_css_reset() {
		...
		$css = file_get_contents( Avangpress_PLUGIN_DIR . 'assets/css/checkbox-reset' . $suffix . '.css' );
	...
	public function get_label_text() {
         ...
         * @ignore
		 */
		$label = (string) apply_filters( 'avangpress_integration_checkbox_label', $label, $integration );
		return $label;
	}
	...
	protected function get_checkbox_attributes() {
         ...
         * @ignore
		 */
		$attributes = (array) apply_filters( 'avangpress_integration_checkbox_attributes', $attributes, $integration );
         ...
         * @ignore
		 */
		$attributes = (array) apply_filters( 'avangpress_integration_' . $slug . '_checkbox_attributes', $attributes, $integration );
	...
	public function get_checkbox_html() {
         ...
         */
        $show_checkbox = (bool) apply_filters( 'avangpress_integration_show_checkbox', $show_checkbox, $integration_slug );
		...
		echo sprintf( '<!-- Mail for WordPress v%s - https://avangemail.com/docs/#/ -->', Avangpress_VERSION );

        /** @ignore */
        do_action( 'avangpress_integration_before_checkbox_wrapper', $this );

        /** @ignore */
		do_action( 'avangpress_integration_'. $this->slug .'_before_checkbox_wrapper', $this );
        ...
        echo sprintf( '<%s class="avangpress-checkbox avangpress-checkbox-%s">', $wrapper_tag, esc_attr( $this->slug ) );
        echo '<label>';
        ...
        /** @ignore */
		do_action( 'avangpress_integration_after_checkbox_wrapper', $this );

        /** @ignore */
		do_action( 'avangpress_integration_'. $this->slug .'_after_checkbox_wrapper', $this );
		echo '<!-- / Mail for WordPress -->';
	...
	public function get_lists() {
		 ...
		 *
		 * @since 2.0
		 * @see Avangpress_Form::get_lists
		 * @ignore
		 */
		$lists = (array) apply_filters( 'avangpress_lists', $lists );

		/**
		 * Filters the Mail lists this integration should subscribe to
		 *
		 * @since 3.0
		 *
		 * @param array $lists
		 * @param Avangpress_Integration $integration
		 */
		$lists = (array) apply_filters( 'avangpress_integration_lists', $lists, $integration );

		/**
		 * Filters the Mail lists a specific integration should subscribe to
		 *
		 ...
		 * @param Avangpress_Integration $integration
		 */
		$lists = (array) apply_filters( 'avangpress_integration_' . $slug . '_lists', $lists, $integration );
	...
	protected function subscribe( array $data, $related_object_id = 0 ) {
		...
		/** @var Avangpress_Mail_Subscriber $subscriber */
		$subscriber = null;
		$result = false;

		// validate lists
		if( empty( $list_ids ) ) {
			$log->warning( sprintf( '%s > No Mail lists were selected', $this->name ) );
			return false;
		}
		 ...
		 * @param array $data
		 */
		$data = apply_filters( 'avangpress_integration_data', $data );
		 ...
		 * @param int $related_object_id
		 */
		$data = apply_filters( "avangpress_integration_{$slug}_data", $data, $related_object_id );

		/**
		 * @ignore
		 * @deprecated 4.0
		 */
		$data = apply_filters( 'avangpress_merge_vars', $data );

		/**
		 * @deprecated 4.0
		 * @ignore
		 */
		$data = apply_filters( 'avangpress_integration_merge_vars', $data, $integration );

		/**
		 * @deprecated 4.0
		 * @ignore
		 */
		$data = apply_filters( "avangpress_integration_{$slug}_merge_vars", $data, $integration );

		$email_type = avangpress_get_email_type();

		$mapper = new Avangpress_List_Data_Mapper( $data, $list_ids );

		/** @var Avangpress_Mail_Subscriber[] $map */
		$map = $mapper->map();

		foreach( $map as $list_id => $subscriber ) {
			$subscriber->status = $this->options['double_optin'] ? 'pending' : 'subscribed';
			$subscriber->email_type = $email_type;
			$subscriber->ip_signup = avangpress_get_request_ip_address();

			/** @ignore (documented elsewhere) */
			$subscriber = apply_filters( 'avangpress_subscriber_data', $subscriber );

			/**
			 * Filters subscriber data before it is sent to Mail. Only fires for integration requests.
			 *
			 * @param Avangpress_Mail_Subscriber $subscriber
			 */
			$subscriber = apply_filters( 'avangpress_integration_subscriber_data', $subscriber );
			 ...
			 * @param int $related_object_id
			 */
			$subscriber = apply_filters( "avangpress_integration_{$slug}_subscriber_data", $subscriber, $related_object_id );

			$result = $mail->list_subscribe( $list_id, $subscriber->email_address, $subscriber->to_array(), $this->options['update_existing'], $this->options['replace_interests'] );
		}
		 ...
		 * @param Avangpress_Mail_Subscriber[] $subscriber_data
		 * @param int $related_object_id
		 */
		do_action( 'avangpress_integration_subscribed', $integration, $subscriber->email_address, $subscriber->merge_fields, $map, $related_object_id );
	 ...
	 *
	 * @see Avangpress_Integration::$checkbox_name
	 * @see Avangpress_Integration::get_lists
	 * @see Avangpress_Integration::checkbox_was_checked
	 *
	 * @return array
	 */
